<template>
	<div class="form-container">
		...
		<a-spin :spinning="loading" class="form">
			<a-form-model class="form" ref="convention" :model="form" :rules="rules" @submit="handleSubmit" v-bind="formItemLayout">
				...
				<div v-show="stepCurrent === 1">
					...
					<a-form-model-item required :label="$t('convention.comType')">
						<a-radio-group v-model="form.institutionType" :disabled="isCheck">
							...
							<a-radio value="OTHER">{{ $t("util.other") }}({{
                                    $t("convention.dm")
                                }})</a-radio>
						</a-radio-group>
					</a-form-model-item>
				...
				<div v-show="stepCurrent === 2">
					<a-form-model-item prop="liaisonId" :label="$t('convention.conPerson1')">
						<a-select v-model="form.liaisonId" showSearch optionFilterProp="label" :filterOption="true" :disabled="isCheck">
							<a-select-option v-for="item in liaisonList" :key="item.id" :value="item.id" :label="`${item.nameZh}${item.nameEnOrPt}`">{{ item.nameZh }}
					...
					<a-form-model-item :label="$t('convention.conPerson2')">
						<a-select v-model="form.liaisonSubId" showSearch optionFilterProp="label" :filterOption="true" :disabled="isCheck">
							<a-select-option v-for="item in liaisonList" :key="item.id" :value="item.id" :label="`${item.nameZh}${item.nameEnOrPt}`">{{ item.nameZh }}
								{{ item.nameEnOrPt }}</a-select-option>
						</a-select>
					</a-form-model-item>
				...
				<div v-show="stepCurrent === 3">
					<a-form-model-item :label="$t('convention.officeNameZh')">
						<a-select v-model="form.activityId" :disabled="isCheck">
							<a-select-option v-for="item in list" :key="item.id" :value="item.id" :label="item.nameZh">{{ item.nameZh }}</a-select-option>
						</a-select>
					</a-form-model-item>
					<a-form-model-item :label="$t('convention.officeNameEn')">
				...
				<div v-show="stepCurrent === 4">
					<div v-show="form.conventionType === 'GENERAL_MEET' || form.conventionType === 'INT_MEET' || form.conventionType === 'MEET_EXHIBIT'">
						<a-form-model-item :label="$t('convention.activeTime')">
							<div class="item">
								<a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.meetingFrom" :disabled="isCheck" placeholder="開始日期" />
								<span>~</span>
								<a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.meetingTo" :disabled="isCheck" placeholder="結束日期" />
							</div>
						</a-form-model-item>
						...
						<a-form-model-item :label="$t('convention.meetingCycle')">
							<a-radio-group v-model="form.meetingCycle" :disabled="isCheck">
								<a-radio value="BIENNIAL">{{ $t("convention.di") }}</a-radio>
								<a-radio value="ANNUAL">{{ $t("convention.dj") }}</a-radio>
								<a-radio value="QUARTERLY">{{ $t("convention.dl") }}</a-radio>
								<a-radio value="OTHER">{{ $t("util.other") }}({{ $t("convention.dm") }})</a-radio>
							</a-radio-group>
						</a-form-model-item>
						<a-form-model-item v-if="form.meetingCycle ==='OTHER'">
							<a-input v-model="form.meetingCycleDescription" :placeholder="$t('convention.dm')" :disabled="isCheck" />
						</a-form-model-item>
						...
						<a-form-model-item :label="$t('convention.meetingAllDay')">
							<div v-for="(item,index) in form.meetingDates" :key='index' class="item">
								...
								<a-button v-if="index === 0" type='primary' icon="plus" shape="circle" @click="form.meetingDates.push({date:'',hours:''})"
								 :disabled="isCheck"></a-button>
						...
						<a-form-model-item :label="$t('convention.func')">
							<div v-for="(item,index) in form.activityDates" :key='index' class="item">
								<a-date-picker v-model="item.date" valueFormat="YYYY-MM-DD" style="width:70%;" :disabled="isCheck" />
								<a-input style='width:20%;' v-model="item.hours" :placeholder="$t('convention.ee')" :disabled="isCheck" />
								<a-button v-if="index === 0" type='primary' icon="plus" shape="circle" @click="form.activityDates.push({date:'',hours:''})"
								 :disabled="isCheck"></a-button>
								<a-button v-else icon="minus" shape="circle" @click="form.activityDates.splice(index,1)" :disabled="isCheck"></a-button>
							</div>
						</a-form-model-item>
						<a-form-model-item :label="$t('convention.meetingArea')">
							<a-radio-group v-model="form.meetingHistory" :disabled="isCheck">
								<a-radio value="TWO_EDITIONS">{{ $t("convention.eg") }}</a-radio>
								<a-radio value="SECOND_EDITION">{{ $t("convention.eh") }}</a-radio>
								<a-radio value="FIRST_EDITION">{{ $t("convention.ei") }}</a-radio>
							</a-radio-group>
							<div v-if="form.meetingHistory === 'TWO_EDITIONS'">
								<div v-for="(item,index) in form.conventionHistory" :key="index" class="item">
									<a-date-picker v-model="item.date" valueFormat="YYYY-MM-DD" :disabled="isCheck" />
									<a-input v-model='item.venue' :placeholder="$t('convention.fd')" :disabled="isCheck" />
								</div>
								<upload :value.sync="form.historyFiles" :disabled="isCheck" />
							</div>
							<div v-if="form.meetingHistory === 'SECOND_EDITION'" class="item">
								<a-date-picker v-model="form.date" valueFormat="YYYY-MM-DD" :disabled="isCheck" />
								<a-input v-model='form.venue' :placeholder="$t('convention.fd')" :disabled="isCheck" />
							</div>
						</a-form-model-item>
						<a-form-model-item :label="$t('convention.parInformation')">
							<div class="item">
								<a-input v-model="form.totalAttendees" :placeholder="$t('convention.fe')" :disabled="isCheck" />
								<a-input v-model="form.attendeesFromMacao" :placeholder="$t('convention.ff')" :disabled="isCheck" />
								<a-input v-model="form.overseasAttendees" :placeholder="$t('convention.fg')" :disabled="isCheck" />
							</div>
							<upload :value.sync="form.attendeeFiles" :disabled="isCheck" />
						</a-form-model-item>
					</div>
					<div v-show="form.conventionType === 'GENERAL_EXHIBIT' || form.conventionType === 'SPECIALTY_EXHIBIT' || form.conventionType === 'MEET_EXHIBIT'">
						<a-form-model-item :label="$t('convention.activeTime')">
							<div class="item">
								<a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.conventionFrom" :disabled="isCheck"
								 placeholder="開始日期" />
								<span>~</span>
								<a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.conventionTo" :disabled="isCheck"
								 placeholder="結束日期" />
							</div>
						</a-form-model-item>
						<a-form-model-item :label="$t('convention.ae')">
							<div v-for="(item,index) in form.openTimes" :key="index" class="item">
								<div class="item sp">
									<a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="item.openFrom" :disabled="isCheck" placeholder="開始日期" />
									<span>~</span>
									<a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="item.openTo" :disabled="isCheck" placeholder="結束日期" />
								</div>
								<a-input v-model="item.total" style="width:20%;" :placeholder="$t('convention.es')" :disabled="isCheck" />
								<a-button v-if="index === 0" type='primary' icon="plus" shape="circle" @click="form.openTimes.push({date:'',time:''})"
								 :disabled="isCheck"></a-button>
								<a-button v-else icon="minus" shape="circle" @click="form.openTimes.splice(index,1)" :disabled="isCheck"></a-button>
							</div>
						</a-form-model-item>
						...
						<a-form-model-item :label="$t('convention.meetingCycle')">
							<a-radio-group v-model="form.activityCycle" :disabled="isCheck">
								...
								<a-radio value="ANNUAL">{{ $t("convention.dj") }}</a-radio>
								<a-radio value="QUARTERLY">{{ $t("convention.dl") }}</a-radio>
								<a-radio value="OTHER">{{ $t("convention.fh") }}</a-radio>
							</a-radio-group>
						</a-form-model-item>
						...
						<a-form-model-item :label="$t('convention.aj')">
							<div class="item">
								<a-input v-model="form.macaoExhibitorsBooth" :placeholder="$t('convention.ep')" :disabled="isCheck" />
								<a-input v-model="form.macaoExhibitors" :placeholder="$t('convention.eq')" :disabled="isCheck" />
								<a-input v-model="form.macaoProfessionalBuyer" :placeholder="$t('convention.er')" :disabled="isCheck" />
								<a-input v-model="form.macaoWatchingThePublic" :placeholder="$t('convention.ex')" :disabled="isCheck" />
							</div>
						</a-form-model-item>
						<a-form-model-item :label="$t('convention.ai')">
							<div class="item">
								<a-input v-model="form.overseasExhibitorsBooth" :placeholder="$t('convention.ep')" :disabled="isCheck" />
								<a-input v-model="form.overseasExhibitors" :placeholder="$t('convention.eq')" :disabled="isCheck" />
								<a-input v-model="form.overseasProfessionalBuyer" :placeholder="$t('convention.er')" :disabled="isCheck" />
								<a-input v-model="form.overseasWatchingThePublic" :placeholder="$t('convention.ex')" :disabled="isCheck" />
							</div>
						</a-form-model-item>
						<a-form-model-item :label="$t('convention.ak')">
							<div class="item">
								<a-input v-model="form.totalExhibitorsBooth" :placeholder="$t('convention.ep')" :disabled="isCheck" />
								<a-input v-model="form.totalExhibitors" :placeholder="$t('convention.eq')" :disabled="isCheck" />
								<a-input v-model="form.totalProfessionalBuyer" :placeholder="$t('convention.er')" :disabled="isCheck" />
								<a-input v-model="form.totalWatchingThePublic" :placeholder="$t('convention.ex')" :disabled="isCheck" />
							</div>
						</a-form-model-item>
						<a-form-model-item :label="$t('convention.al')">
							<a-input v-model="form.overSeasBuyers" style="margin-bottom:10px;" :disabled="isCheck" />
				...
				<div v-show="stepCurrent === 5">
					...
					<div v-if="form.accommodationSupport">
						<a-form-model-item :label="$t('convention.fz')">
							<div v-for="(item,index) in form.meetingRooms" :key="index" class="item">
								<a-date-picker v-model="item.date" valueFormat="YYYY-MM-DD" style="width:48%;" :placeholder="$t('convention.fj')"
								 :disabled="isCheck" />
								<a-input v-model="item.rooms" :placeholder="$t('convention.fk')" :disabled="isCheck" />
								<a-button v-if="index === 0" type='primary' icon="plus" shape="circle" @click="form.meetingRooms.push({date:'',rooms:''})"
								 :disabled="isCheck"></a-button>
								<a-button v-else icon="minus" shape="circle" @click="form.meetingRooms.splice(index,1)" :disabled="isCheck"></a-button>
							</div>
						</a-form-model-item>
						<a-form-model-item :label="`${$t('convention.start5')}${$t('convention.fl')}`">
							<a-input v-model="form.meetingStar5Rooms" :disabled="isCheck" />
						</a-form-model-item>
						<a-form-model-item :label="`${$t('convention.start5')}${$t('convention.fm')}`">
							<a-input v-model="form.meetingStar5Name" :disabled="isCheck" />
						</a-form-model-item>
						<a-form-model-item :label="`${$t('convention.start4')}${$t('convention.fl')}`">
							<a-input v-model="form.meetingStar4Rooms" :disabled="isCheck" />
						</a-form-model-item>
						<a-form-model-item :label="`${$t('convention.start4')}${$t('convention.fm')}`">
							<a-input v-model="form.meetingStar4Name" :disabled="isCheck" />
						</a-form-model-item>
						<a-form-model-item :label="`${$t('convention.start3')}${$t('convention.fl')}`">
							<a-input v-model="form.meetingStar3Rooms" :disabled="isCheck" />
						</a-form-model-item>
						<a-form-model-item :label="`${$t('convention.start3')}${$t('convention.fm')}`">
							<a-input v-model="form.meetingStar3Name" :disabled="isCheck" />
						</a-form-model-item>
						<a-form-model-item :label="`${$t('convention.otherstar')}${$t('convention.fl')}`">
							<a-input v-model="form.meetingOtherRooms" :disabled="isCheck" />
						</a-form-model-item>
						<a-form-model-item :label="`${$t('convention.otherstar')}${$t('convention.fm')}`">
					...
					<a-form-model-item v-if='form.packageSupport' :label="$t('convention.fo')">
						<div v-for="(item,index) in form.conventionDinings" :key="index" class="item">
							<a-date-picker v-model='item.date' valueFormat='YYYY-MM-DD' :placeholder="$t('convention.bg')" class="full"
							 :disabled='isCheck' />
							<a-input v-model='item.attendees' :placeholder="$t('convention.bh')" :disabled='isCheck' />
							<a-input v-model='item.venue' :placeholder="$t('convention.bi')" :disabled='isCheck' />
							<a-button v-if="index === 0" type='primary' icon="plus" shape="circle" @click="form.conventionDinings.push({ date: '', attendees: '', venue: '' })"
							 :disabled="isCheck"></a-button>
							<a-button v-else icon="minus" shape="circle" @click="form.conventionDinings.splice(index,1)" :disabled="isCheck"></a-button>
						</div>
					</a-form-model-item>
					<p>{{$t('convention.bj')}}</p>
					...
					<a-form-model-item :label="$t('convention.br')">
						<a-radio-group v-model="form.localTransportationSupport" :disabled="isCheck">
							<a-radio :value="true">{{$t('util.yes')}}</a-radio>
							<a-radio :value="false">{{$t('util.no')}}</a-radio>
						</a-radio-group>
					</a-form-model-item>
					...
					<p>{{$t('convention.bu')}}<span>{{$t('convention.ci')}}</span></p>
					...
					<p>{{$t('convention.bv')}}<span>{{$t('convention.ci')}}</span></p>
					...
					<div v-if="form.exVenueRentalSupport">
						<a-form-model-item :label="$t('convention.gb')">
							<div class="item">
								<a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.buildVenueFrom" placeholder="搭建開始日期"
								 :disabled="isCheck" />
								<span>~</span>
								<a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.buildVenueTo" placeholder="搭建結束日期" :disabled="isCheck" />
							</div>
							<div class="item">
								<a-input v-model="form.buildGroupArea" :placeholder="$t('convention.ag')" :disabled="isCheck" />
								<a-input v-model="form.buildPrice" :placeholder="$t('convention.cf')" :disabled="isCheck" />
								<a-input v-model="form.buildTotalPrice" :placeholder="$t('convention.cg')" :disabled="isCheck" />
							</div>
						</a-form-model-item>
						<a-form-model-item :label="$t('convention.gc')">
							<div class="item">
								<a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.exhibitionVenueFrom" placeholder="展覽開始日期"
								 :disabled="isCheck" />
								<span>~</span>
								<a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.exhibitionVenueTo" placeholder="展覽結束束日期"
								 :disabled="isCheck" />
							</div>
							<div class="item">
								<a-input v-model="form.exhibitionGroupArea" :placeholder="$t('convention.ag')" :disabled="isCheck" />
								<a-input v-model="form.exhibitionPrice" :placeholder="$t('convention.cf')" :disabled="isCheck" />
								<a-input v-model="form.exhibitionTotalPrice" :placeholder="$t('convention.cg')" :disabled="isCheck" />
							</div>
						</a-form-model-item>
						<a-form-model-item :label="$t('convention.gd')">
							<div class="item">
								<a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.apartVenueFrom" placeholder="拆展開始日期"
								 :disabled="isCheck" />
								<span>~</span>
								<a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.apartVenueTo" placeholder="拆展結束束日期"
								 :disabled="isCheck" />
							</div>
							<div class="item">
								<a-input v-model="form.apartGroupArea" :placeholder="$t('convention.ag')" :disabled="isCheck" />
								<a-input v-model="form.apartPrice" :placeholder="$t('convention.cf')" :disabled="isCheck" />
								<a-input v-model="form.apartTotalPrice" :placeholder="$t('convention.cg')" :disabled="isCheck" />
							</div>
						</a-form-model-item>
					</div>
					<p>{{$t('convention.au')}}</p>
					...
					<div v-if="form.exAccommodationSupport">
						<a-form-model-item :label="$t('convention.fz')">
							<div v-for="(item,index) in form.exMeetingRooms" :key="index" class="item">
								<a-date-picker v-model="item.date" valueFormat="YYYY-MM-DD" style="width:48%;" :placeholder="$t('convention.fj')"
								 :disabled="isCheck" />
								<a-input v-model="item.rooms" :placeholder="$t('convention.fk')" :disabled="isCheck" />
								<a-button v-if="index === 0" type='primary' icon="plus" shape="circle" @click="form.exMeetingRooms.push({date:'',rooms:''})"
								 :disabled="isCheck"></a-button>
								<a-button v-else icon="minus" shape="circle" @click="form.exMeetingRooms.splice(index,1)" :disabled="isCheck"></a-button>
							</div>
						</a-form-model-item>
						<a-form-model-item :label="`${$t('convention.start5')}${$t('convention.fl')}`">
					...
					<p>{{$t('convention.bp')}}</p>
					<a-form-model-item :label="$t('convention.bq')">
						<a-radio-group v-model="form.exPromotionMarketingSupport" :disabled="isCheck">
							<a-radio :value="true">{{$t('util.yes')}}</a-radio>
							<a-radio :value="false">{{$t('util.no')}}</a-radio>
						</a-radio-group>
					</a-form-model-item>
					<p>{{$t('convention.bx')}}</p>
					<a-form-model-item :label="$t('convention.br')">
						<a-radio-group v-model="form.exLocalTransportationSupport" :disabled="isCheck">
							<a-radio :value="true">{{$t('util.yes')}}</a-radio>
							<a-radio :value="false">{{$t('util.no')}}</a-radio>
						</a-radio-group>
					</a-form-model-item>
				...
				<div v-show="stepCurrent === 6">
					<a-form-model-item :label="$t('convention.cl')">
						...
						<div v-if="form.otherGovernmentSupport">
							<div v-for="(item,index) in form.conventionOtherGovs" :key="item.id" class="item">
								<a-input v-model="item.name" :placeholder="$t('convention.cn')" :disabled="isCheck" />
								<a-input v-model="item.applicationContent" :placeholder="$t('convention.co')" :disabled="isCheck" />
								<a-button v-if="index === 0" type='primary' icon='plus' shape="circle" @click="form.conventionOtherGovs.push({name:'',applicationContent:''})"
								 :disabled="isCheck"></a-button>
								<a-button v-else icon='minus' shape="circle" @click="form.conventionOtherGovs.splice(index,1)" :disabled="isCheck"></a-button>
							</div>
						</div>
					</a-form-model-item>
				</div>
				...
				<a-form-model-item>
					<a-button v-show="stepCurrent > 0" type="primary" @click="stepCurrent--" style="margin-right:12px">上一步</a-button>
					<a-button v-show="stepCurrent < 7" type="primary" :disabled="timeNext > 0" @click="stepCurrent++">
						{{ timeNext > 0 ? `(${timeNext}S)` : "下一步" }}</a-button>
					<a-button v-show="stepCurrent === 7" type="primary" :class="isSubmit ? 'none' : ''" @click="handleSubmit">{{ $t("attend.bo") }}</a-button>
				</a-form-model-item>
			</a-form-model>
		</a-spin>
	</div>
