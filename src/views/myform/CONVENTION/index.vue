<template>
	<div class="form-container">
		<a-steps v-model="stepCurrent" direction="vertical" size="small">
			...
			<a-step :title="$t('convention.fi')" disabled/>
			...
			<a-form-model class="form" ref="convention" :model="form" :rules="rules" @submit="handleSubmit" v-bind="formItemLayout">
				...
				<div v-show="stepCurrent === 4">
					<div v-show="form.conventionType === 'GENERAL_MEET' || form.conventionType === 'INT_MEET' || form.conventionType === 'MEET_EXHIBIT'">
						...
						<a-form-model-item :label="$t('convention.func')">
							<div v-for="(item,index) in form.activityDates" :key='index' class="item">
								<a-date-picker v-model="item.date" valueFormat="YYYY-MM-DD" style="width:70%;" :disabled="isCheck" />
								<a-input style='width:20%;' v-model="item.hours" :placeholder="$t('convention.ee')" :disabled="isCheck" />
								<a-button v-if="index === 0" type='primary' icon="plus" shape="circle" @click="form.activityDates.push({date:'',hours:''})" :disabled="isCheck"></a-button>
					...
					<div v-show="form.conventionType === 'GENERAL_EXHIBIT' || form.conventionType === 'SPECIALTY_EXHIBIT' || form.conventionType === 'MEET_EXHIBIT'">
						<a-form-model-item :label="$t('convention.activeTime')">
                            <div class="item">
                                <a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.conventionFrom" :disabled="isCheck" placeholder="開始日期" />
                                <span>~</span>
                                <a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.conventionTo" :disabled="isCheck" placeholder="結束日期" />
                           </div>
						</a-form-model-item>
						<a-form-model-item :label="$t('convention.ae')">
							<div v-for="(item,index) in form.openTimes" :key="index" class="item">
								...
								<a-button v-if="index === 0" type='primary' icon="plus" shape="circle" @click="form.openTimes.push({date:'',time:''})" :disabled="isCheck"></a-button>
				...
				<div v-show="stepCurrent === 5">
					...
					<div v-if="form.accommodationSupport">
						<a-form-model-item :label="$t('convention.fz')">
                            <div v-for="(item,index) in form.meetingRooms" :key="index" class="item">
                                <a-date-picker v-model="item.date" valueFormat="YYYY-MM-DD" style="width:48%;" :placeholder="$t('convention.fj')" :disabled="isCheck"/>
                                <a-input v-model="item.rooms" :placeholder="$t('convention.fk')" :disabled="isCheck" />
                                <a-button v-if="index === 0" type='primary' icon="plus" shape="circle" @click="form.meetingRooms.push({date:'',rooms:''})" :disabled="isCheck"></a-button>
					...
					<a-form-model-item v-if='form.packageSupport' :label="$t('convention.fo')">
						<div v-for="(item,index) in form.conventionDinings" :key="index" class="item">
							<a-input :placeholder="$t('convention.bg')" />
							<a-input :placeholder="$t('convention.bh')" />
							<a-input :placeholder="$t('convention.bi')" />
							<a-button v-if="index === 0" type='primary' icon="plus" shape="circle" @click="form.conventionDinings.push({ date: '', attendees: '', venue: '' })" :disabled="isCheck"></a-button>
				...
				<div v-show="stepCurrent === 6">
					...
					<div v-if="form.exVenueRentalSupport">
                        <a-form-model-item :label="$t('convention.fu')">
						<div class="item">
                            <div class="item sp">
                                <a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.buildVenueFrom" placeholder="搭建開始日期" :disabled="isCheck" />
                                <span>~</span>
                                <a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.buildVenueTo" placeholder="搭建結束日期" :disabled="isCheck" />
                            </div>
                            <div class="item sp">
                                <a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.exhibitionVenueFrom" placeholder="展覽開始日期" :disabled="isCheck" />
                                <span>~</span>
                                <a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.exhibitionVenueTo" placeholder="展覽結束束日期" :disabled="isCheck"/>
                            </div>
                            <div class="item sp">
                                <a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.exhibitionVenueFrom" placeholder="拆展開始日期" :disabled="isCheck" />
                                <span>~</span>
                                <a-date-picker class="full" valueFormat="YYYY-MM-DD" v-model="form.exhibitionVenueTo" placeholder="拆展結束束日期" :disabled="isCheck" />
                            </div>
						</div>
                        </a-form-model-item>
                        <a-form-model-item :label="$t('convention.ag')">
                            <div class="item">
                                <a-input v-model="form.buildGroupArea" :disabled="isCheck"/>
                                <a-input v-model="form.exhibitionGroupArea" :disabled="isCheck"/>
                                <a-input v-mdoel="form.apartGroupArea" :disabled="isCheck"/>
                            </div>
                        </a-form-model-item>
                        <a-form-model-item :label="$t('convention.cf')">
                            <div class="item">
                                <a-input v-model="form.buildPrice" :disabled="isCheck"/>
                                <a-input v-model="form.exhibitionPrice" :disabled="isCheck"/>
                                <a-input v-model="form.apartPrice" :disabled="isCheck"/>
                            </div>
                        </a-form-model-item>
                        <a-form-model-item :label="$t('convention.cg')">
                            <div class="item">
                                <a-input v-model="form.buildTotalPrice" :disabled="isCheck"/>
                                <a-input v-model="form.exhibitionTotalPrice" :disabled="isCheck"/>
                                <a-input v-model="form.apartTotalPrice" :disabled="isCheck"/>
                            </div>
                        </a-form-model-item>
                    </div>
					<p>{{$t('convention.au')}}</p>
					...
					<div v-if="form.exAccommodationSupport">
						...
						<div v-for="(item,index) in form.exMeetingRooms" :key="index" class="item">
                                <a-date-picker v-model="item.date" valueFormat="YYYY-MM-DD" style="width:48%;" :placeholder="$t('convention.fj')" :disabled="isCheck" />
                                <a-input v-model="item.rooms" :placeholder="$t('convention.fk')" :disabled="isCheck" />
                                <a-button v-if="index === 0" type='primary' icon="plus" shape="circle" @click="form.exMeetingRooms.push({date:'',rooms:''})" :disabled="isCheck"></a-button>
					...
					<a-form-model-item :label="$t('convention.cl')">
						...
						<div v-for="(item,index) in form.conventionOtherGovs" :key="item.id" class="item">
							...
							<a-button v-if="index === 0" type='primary' icon='plus' shape="circle" @click="form.conventionOtherGovs.push({name:'',applicationContent:''})" :disabled="isCheck"></a-button>
